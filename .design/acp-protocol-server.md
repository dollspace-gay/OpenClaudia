...
### Q1: `glob` and `grep` tool mapping
**Decision**: Delegate via `terminal/create` running `find`/`rg` commands (option A). This respects the ACP client's permission model, `rg` is fast enough that terminal overhead is negligible, and it's the simplest implementation with no new code paths. Option B (many `fs/read_text_file` calls) would be extremely chatty; option C (local execution) breaks the ACP delegation contract.

### Q2: MCP tool passthrough in ACP mode
**Decision**: Yes, expose MCP tools during ACP sessions. MCP tools are explicitly configured by the user in `.mcp.json` — they've opted in. Many MCP tools provide capabilities with no ACP equivalent (database queries, custom APIs). Excluding them would make ACP mode strictly less capable than interactive mode for no safety gain. The hybrid model is documented: ACP client methods for file/terminal, local execution for MCP tools and internal tools (memory, tasks).

### Q3: Upstream registration in acpx built-in agent registry
**Decision**: Yes, pursue after implementation lands. Canonical command: `openclaudia acp` (matches the pattern: `openclaw acp`, `gemini --acp`, `kimi acp`). Submit a PR to the acpx repo once end-to-end testing passes. Manual config (`~/.acpx/config.json`) works in the meantime.
