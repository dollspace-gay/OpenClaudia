...
- auth: `openclaudia auth` now writes the shared Claude-compatible `.credentials.json` store used by chat/TUI/proxy, and `auth --status` reports that chat-usable credential file.
