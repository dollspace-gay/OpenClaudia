...
### Added
- coordinator infrastructure — Phase 1 (crosslink #507). New `src/coordinator/` module with the shape the Phase 2 dispatch loop needs, tests in place, no wiring yet. `Coordinator::dispatch` returns `CoordinatorError::NotImplemented` until Phase 2 adds the teammate spawn path. `TaskQueue` does dependency-aware readiness polling (O(N) per call — fine for <50 tasks/run) with cycle detection on both `submit` (blocked at `UnknownTask`-only since new tasks can only reference existing ones) and `add_dependency` (walks the existing graph from `to` looking for `from` — rejects the edge when it'd close a loop). `Teammate` carries an `AgentColor` from a fixed 7-slot rainbow palette matching Claude Code's ordering so transcripts viewed in either harness color-code identically; colors wrap after the 7th teammate with id prefix as the tiebreaker. `LeaderPermissionBridge` FIFO queue + per-teammate `(TeammateId, tool_name, target)` always-allow cache so one teammate's `a` reply can't widen permissions for another teammate, tool, or target (matches CC). 23 new tests
- memdir MEMORY.md entrypoint — Phase 1 (crosslink #508). New `src/memdir/` module. `load_entrypoint(cwd)` discovers `<cwd>/MEMORY.md`, then `<cwd>/.openclaudia/MEMORY.md`, then `~/.openclaudia/MEMORY.md` (first hit wins). Root path first means repos shared with Claude Code users pick up the same file. Truncation rules match CC exactly: `MAX_ENTRYPOINT_LINES = 200`, `MAX_ENTRYPOINT_BYTES = 25 000`, lines-first then bytes-second, UTF-8-safe byte truncation (walks backward to a char boundary, never splits a codepoint), trailing "truncated" suffix so the model knows some entries aren't visible. Zero wire-up — callers opt in via `load_entrypoint` + `SystemPromptBlocks` injection; no existing prompt flow changes. 10 new tests
- centralized `SessionState` module — Phase 0 (crosslink #510). New `src/state/` with `SessionState` struct grouped into 7 sub-categories (`Identity` / `Conversation` / `UiState` / `ModesState` / `PermissionsState` / `BudgetsState` / `TranscriptState`), `SessionId` newtype, `EffortLevel` enum with wire-format `as_str()` / `parse()`, `AgentMode` library-visible copy of the binary-side enum. `StateStore` wrapper exposes `Arc<RwLock<SessionState>>` + `tokio::sync::broadcast` channel with granular `StateEvent` variants; `StateWriteGuard` accumulates noted events and flushes on drop so a panic mid-mutation can't leave subscribers out of sync. `SessionStateV1` carries a schema-version tag; `persist::decode` rejects future versions and decodes legacy-shape files as version 0 so the migrations framework (#506) can dispatch on it. Pure additive — nothing consumes it yet; Phase 1+ migrates fields off `TuiSession` / `ChatSession`. 22 new tests
- services layer (crosslink #511 partial). New `src/services/` module organizes cross-cutting concerns behind traits. `ServiceRegistry` holds `Arc<dyn AnalyticsSink>` + `Arc<dyn FeatureFlagSource>` and is clone-cheap. Default impls are `NoopAnalytics` + empty `StaticFlags` — nothing changes on the hot path until a caller installs a real sink. Concrete impls shipped: `NoopAnalytics`, `TracingAnalytics` (tracing `target: "openclaudia::analytics"` forwarder), `StaticFlags` (HashMap + `OPENCLAUDIA_FEATURE_<NAME>` env override with truthy/falsy parse). Typed `AnalyticsEvent` enum forces sinks to handle every variant — no stringly-typed silent-drop surface. 13 new tests
- plugin enterprise policy (crosslink #514 partial). New `src/plugins/policy.rs` with `PluginPolicy` (strict_known_marketplaces, blocked_marketplaces, skipped_marketplaces, skipped_plugins, managed flag), `check_marketplace_allowed` (blocklist takes precedence over allowlist; missing allowlist means "no enforcement"; empty allowlist means "deny all"), `is_plugin_skipped`, `is_marketplace_skipped`, and source-identity matching (case-insensitive GitHub repos, `.git` canonicalization, ref/path wildcarding when the rule omits them). `PluginManager::add_marketplace_from_{directory,git}_with_policy` enforce before any filesystem side effect — matches Claude Code's "check before download" guarantee. New `PluginError::PolicyRejected { reason, scope }` variant where `scope` distinguishes managed vs user-preference rejections. 12 new tests
- rich TUI components: new `src/tui/components/` module with an `Overlay` trait + `ActiveOverlay` single-slot state on `App`. Ships two components — `HelpOverlay` (scrollable keybindings cheatsheet, opened via `/help` or `?`) and `LogSelector` (session-picker for `/resume` without args; rows sourced from `transcript::list_transcripts`, arrow/vim keys to move, Enter to resume). Plumbing: overlay consumes every keystroke while open so stray keys can't leak to the chat input; `OverlayAction` return value keeps the event loop as the single owner of App-level state changes; `centered_rect` helper positions overlays. 13 new tests (crosslink #515 partial — ModelPicker / ModePicker / NotificationBar / AskUserQuestion dialog remain; plumbing is ready)
- `/agents` slash command lists every subagent type (`general-purpose`, `explore`, `plan`, `claude-code-guide`, `coordinator`) with a one-line description so users can pick one by name before invoking the `task` tool. New `AgentType::ALL` / `name()` / `description()` surface area; round-trip test guards against name/parse drift as new agents are added (crosslink #512 partial — OC already covered 65+ CC commands; this fills the highest-value remaining gap)
...
### Fixed
- permissions: `PermissionManager::unrestricted()` now skips prompts/rules without bypassing hard safety checks for denylisted Bash commands, `dangerously_disable_sandbox` tool args, `.git` writes, or `.claude/settings.json`.
